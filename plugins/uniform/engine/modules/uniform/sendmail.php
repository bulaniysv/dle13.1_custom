...
if(!function_exists('RequestPost')) { function RequestPost($url, $data = array(), $options = array(), &$headers = array()) {
    $headers = array();
    $defaults = array(
        CURLOPT_POST => 1,
        CURLOPT_HEADER => 0,
...
foreach ($arMails as $email) {
  ...
  if ($cfg['sendToSender']) {
    ...
    $msg = $parsedMessage; 
		if(!$helpdeskApi) $mail->send($email, $parsedHeader, $parsedMessage);
	} else {
		$msg = $message;
    if(!$helpdeskApi) $mail->send($email, $emailHeader, $message);
	}
...
if($helpdeskApi) {
  $data = array(
    'email' => $_POST['email'], 'text' => $_POST['textarea'], 'name' => $_POST['email'], 
    'title' => 'Обращение с сайта: '.preg_replace('/^https?:\/\//is', '', preg_replace('/\/+?$/is', '', $_SERVER['HTTP_REFERER'])), 
  );
  $helpdeskApi = RequestPost('https://tech-service.gb.net/MApi/AddTicket/', $data);
  //echo($helpdeskApi);
}
