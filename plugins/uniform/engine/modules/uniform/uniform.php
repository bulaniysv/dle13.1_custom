	...
	if (file_exists(TEMPLATE_DIR . '/uniform/' . $cfg['templateFolder'] . '/form.tpl')) {
		...
		if ($isPost) {
			...
			foreach ($_POST as $k => $val) {
				...
				if (in_array($k, $arRequired) && $k != 'textarea' && ($val == '' || !isset($val))) {
        ...
        if($k == 'textarea' && strlen($val) < 15) {
        	// Если поле обязательное, но значение не установлено — значит произошла ошибка заполнения формы
					$error = true;
					// Показываем то, что заключено в теги ошибок.
					$tpl->copy_template = str_replace("[uf_error_min_{$k}]", '', $tpl->copy_template);
					$tpl->copy_template = str_replace("[/uf_error_min_{$k}]", '', $tpl->copy_template);
				} else {
					// Удалем теги, которые не должны показываться
					$tpl->copy_template = preg_replace("'\\[uf_error_min_{$k}\\](.*?)\\[/uf_error_min_{$k}\\]'is", '', $tpl->copy_template);
				}
