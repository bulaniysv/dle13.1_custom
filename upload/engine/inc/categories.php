...
if( $action == "add" ) {
	...
	function DeleteSubcategories($parentid) {
		...
		while ( $subcategory = $db->get_row( $subcategories ) ) {
			...
			$row = $db->super_query( "SELECT count(*) as count FROM " . PREFIX . "_post WHERE category regexp '[[:<:]]({$subcategory['id']})[[:>:]]'" );
			...
			if( $row['count'] ) {
				if( $_POST['subaction'] == 2 AND is_array( $_REQUEST['new_category'] ) ) {
					if( !in_array( $subcategory['id'], $_REQUEST['new_category'] ) ) {
						...
						$result = $db->query( "SELECT id, category FROM " . PREFIX . "_post WHERE category regexp '[[:<:]]({$subcategory['id']})[[:>:]]'" );
				...
				if( $_POST['subaction'] == 3) {
					...
					$result = $db->query( "SELECT id FROM " . PREFIX . "_post WHERE category regexp '[[:<:]]({$subcategory['id']})[[:>:]]'" );
	...
	$row = $db->super_query( "SELECT count(*) as count FROM " . PREFIX . "_post WHERE category regexp '[[:<:]](".implode("|",get_sub_cats($catid)).")[[:>:]]'" );
