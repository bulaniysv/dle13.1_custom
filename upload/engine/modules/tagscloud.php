...
if ($tpl->result['tags_cloud'] === false) {
	...
	foreach ($list as $value) {
		...
		if (trim($value['tag']) != "" ) {
			...
			$url_tag = str_replace("&amp;", "&", $value['tag']);
		
			if ($config['allow_alt_url'] )
	        	$tags[] = "<span class=\"{$value['size']}\"><a href=\"".$config['http_home_url']."tags/".urlencode($url_tag)."/\" title=\"".$lang['tags_count']." ".$value['count']."\">".$value['tag']."</a></span>";
			else
				$tags[] = "<span class=\"{$value['size']}\"><a href=\"$PHP_SELF?do=tags&amp;tag=".urlencode($url_tag)."\" title=\"".$lang['tags_count']." ".$value['count']."\">".$value['tag']."</a></span>";
